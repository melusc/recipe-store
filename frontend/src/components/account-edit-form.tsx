...
export function AccountEditForm({
	...
	return (
		...
		>
			...
			{adminForm && requestUser.permissionToChangeRole(user) && (
				<>
					<div>
						...
						>
							{[...UserRolesLabels].map(role => (
								<option value={role} selected={user.roleLabel === role}>
									{role}
								</option>
							))}
						</select>
			...
			{!adminForm && (
				<>
					<PasswordRequirements required={false} />
					...
					<div>
						<label for="new-password-repeat" class="form-label">
							Repeat new password:
						</label>
						<input
							type="password"
