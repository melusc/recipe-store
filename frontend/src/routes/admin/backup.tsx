	...
	(
		...
		body: (
			<CenteredMain>
				...
				<section>
					<h2>Create Backup</h2>
					<FormError errors={error?.download} />
					<form
				...
				<section class="mt-4">
					...
					<FormError errors={error?.restore} />
					{restoreSuccess && (
						<div class="alert alert-success" role="alert">
							Successfully restored data.
						</div>
					)}
					{error?.skippedUsers && error.skippedUsers.length > 0 && (
						<div class="alert alert-warning" role="alert">
							Skipped following duplicate users:
							<ul>
								{error.skippedUsers.map(user => (
									<li>{user}</li>
								))}
							</ul>
						</div>
					...
					<div class="alert alert-info" role="alert">
						This does not reset the store. If you want to restore to an older
						version, manually delete the data-directory first.
					</div>
					<form
